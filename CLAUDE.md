...
This repository is the `rimo` plugin marketplace. Each plugin lives in its own directory under `plugins/<name>/` with its own `.claude-plugin/plugin.json`; the marketplace catalog is `.claude-plugin/marketplace.json` at the repo root.
